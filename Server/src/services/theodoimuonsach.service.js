...
module.exports = class BorrowBook {
  async getAllForUser(MaDocGia) {
    const result = await muonSachModel
  ...
  async sendEmailToUser(muonId, email) {
    const muon = await muonSachModel.findById(muonId).populate("MASACH");
  ...
  async addBorrow(data, docgiaId) {
    ...
    if (borrowNew) {
      ...
      const docgia = await muonSachModel
        .findById(borrowNew._id)
        .populate("MADOCGIA", "EMAIL");
  ...
  async deleteBorrowForUser(muonId) {
    const borrowDelete = await muonSachModel.findOneAndDelete({
      _id: muonId,
      TrangThai: "Chờ lấy",
    });
    if (borrowDelete) {
  ...
  async deleteBorrowForAdmin(muonId) {
    ...
    if (borrow.TrangThai === "Chờ lấy") {
      return {
        message:
          "Bạn không được phép xóa phiếu mượn khi đang trong trạng thái chờ lấy.",
      };
    }
  ...
  async updateBorrowForAdmin(data, muonId, nhanVienId) {
    ...
    if (
      kiemTra.TrangThai === "Đã trả" &&
      (data.TrangThai === "Chờ lấy" || data.TrangThai === "Đã lấy")
    ) {
      return {
        message: "Sách đã được trả. Không thể cập nhật lại trạng thái.",
      };
    }
    if (data.TrangThai == "Đã lấy") {
      ...
      const chiTietMuon = await muonSachModel
        .findOne({ _id: muonId })
        .populate("MASACH")
      ...
      const muon = await muonSachModel.findByIdAndUpdate(
        muonId,
        {
          $set: {
            TrangThai: data.TrangThai,
