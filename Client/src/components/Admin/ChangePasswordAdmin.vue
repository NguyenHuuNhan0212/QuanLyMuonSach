<template>
  <main class=" pt-5" data-aos="fade-up" data-aos-duration="1000">
...
const changePassword = async () =>{
    try{
        if(form.value.newPassword !== form.value.validationPassword){
            ElMessage.warning('Xác nhận mật khẩu không đúng')
            return
        }
        if(form.value.newPassword.length < 6){
            ElMessage.warning('Mật khẩu phải dài hơn 6 ký tự.')
            return
        }
        const data = {
        ...
        const result = await userStore.changePasswordAdmin(data)
